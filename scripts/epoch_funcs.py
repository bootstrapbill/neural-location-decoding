...
def epoch_localizers(raw, pID, tmin=-0.2, tmax=1, get_epochs = 0):
    ...
    offset = 1/120 # used to correct event timing as triggers were sent 1 frame late        
...
def epoch_tf(data, pID):
    ...
    n_cycles = np.logspace(np.log10(3), 
                           np.log10(10),
                           len(freqs));
    power = tfr_array_morlet(data, sfreq = 512, freqs=freqs, 
                             n_cycles=n_cycles, use_fft=True, output='power')
    power = np.ndarray.copy(power[:,:,:,512:1127]) # ~ -200-1000 ms 
    
    return power

def epoch_loc_and_motion(raw, pID, tmin=-0.2, tmax=0.4, synth=False):
    ...
    for rep in [0, 1]:
        ...
        for ind in np.linspace(1,40,40).astype('int'):
            ...
            for i in range(temp.shape[0]):
                ...
                # add in average response to localisers in each successive position 
                # shifted by relevant lag
                for x, subPos in enumerate(positions[ind:(ind + 39)]):
                    
                    index = np.argwhere(tempYTest == subPos)
                    addedSignal = np.hstack((np.nan * np.empty((17, indices[x])), np.mean(tempTest[index, :, :-indices[x]], axis = 0).squeeze()))
                    
                    superTrial[x + 1, :, :] = addedSignal
            
                temp[i, :, :]  = np.nanmean(superTrial, axis = 0) 
